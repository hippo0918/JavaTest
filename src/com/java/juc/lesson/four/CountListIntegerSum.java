...
public class CountListIntegerSum {
	private static long sum;//存放整数的和
	private static CyclicBarrier barrier;//障栅集合点(同步器)
	 ...
	 * @return
	 */
	public static long getIntegerSum(List<Integer> list,int threadCounts){
		...
		for(int i=0;i<threadCounts;i++){
			...
			if(i==threadCounts-1){//最后一个线程承担剩下的所有元素的计算
				exec.execute(new CountListIntegerSum.SubIntegerSumTask(list.subList(i*len,list.size())));
			}else{
	 ...
	 *
	 */
	public static class SubIntegerSumTask implements Runnable{
		...
		public void run() {
			...
			synchronized(this){//在CountListIntegerSum对象上同步
				sum+=subSum;
			}
			try {
				barrier.await();//关键，使该线程在障栅处等待，直到所有的线程都到达障栅处
